...
const FilterScreen = ({ navigation }) => {
  ...
  // Redux se current selected id
  const filterCategoryId = useSelector(state => state.FilterCategorySlice.selectedCategory);

  // ⬇️ Local state (Temp selection)
  const [selectedLocalCategory, setSelectedLocalCategory] = useState(filterCategoryId);
  ...
  const handleClearFilters = () => {
    setSelectedLocalCategory(null);   // local state clear
    dispatch(clearSelectedCategoryId(null))
  };
    ...
    <CustomContainer>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={styles.optionRow}
            onPress={() => setSelectedLocalCategory(item._id)}  // ⬅️ Redux नहीं, सिर्फ Local Update
          >
            <Text style={st.filterText}>{item.title}</Text>
            <View style={styles.radioOuter}>
              {selectedLocalCategory === item._id && <View style={styles.radioInner} />}
            </View>
          </TouchableOpacity>
        )}
        ListHeaderComponent={() => (
      ...
      <View style={styles.bottomButtons}>
        <TouchableOpacity style={styles.clearBtn} onPress={handleClearFilters}>
          <Text style={st.errorText}>{selectedLocalCategory ? "Clear Filters (1)" : "Clear Filters"}</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={[styles.showBtn, { opacity: selectedLocalCategory === null ? 0.6 : 1 }]}
          disabled={selectedLocalCategory === null}
          onPress={() => {
            // ⬇️ AB REDUX UPDATE HOGA
            dispatch(setSelectedCategoryId(selectedLocalCategory));
            navigation.navigate('ExploreScreen');
          }}
        >
          <Text style={st.filterText}>Show Results</Text>
