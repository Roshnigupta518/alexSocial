...
const HomeScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    // jab hasMore true ho jaye & isFocused ho, tab fetch karo
    if (isFocused && hasMore && pagination.skip === 0) {
      setLoading(true);
      getStoryHandle();
    }
  }, [isFocused, hasMore, pagination.skip]);
  ...
  const _onViewableItemsChanged = useCallback(({ viewableItems }) => {
    ...
    // 🔥 LOAD MORE WHEN 2 ITEMS LEFT
    if (
      hasMore &&
      !pagination.isLoading &&
      postArray.length - index <= 2
    ) {
      setPagination(prev => ({
        ...prev,
  ...
  }, [postArray.length, hasMore, pagination.isLoading]);
  ...
  useFocusEffect(
    useCallback(() => {
      if (
        ...
        currentItemIndex > 0 &&
        currentItemIndex < postArray.length
      ) {
      ...
      <View style={styles.container}>
        ...
        <View
          ...
          <FlatList
            ...
            onViewableItemsChanged={_onViewableItemsChanged}
            viewabilityConfig={_viewabilityConfig}
            removeClippedSubviews={Platform.OS === 'ios' ? false : true} // ← ये false रखो (video unmount होने से बचता है)
            windowSize={11} // 5 बहुत कम था, 11-13 safe range है
            ...
            keyExtractor={(item, index) => {
              if (item.type === 'event') {
                return `event_${item.eventData?._id || index}`;
              }
              return `post_${item.postData?._id || index}`;
            }}
            extraData={{ screenHeight }}
            ListEmptyComponent={() => {
              if (shouldShowLoader) {
